 ...
 * [uid] is used to restore and reopen the dialog if instantiated through [SamaActivity.manageDialog] (-1 is ignored).
...
abstract class SamaDialogFragment(
    ...
    protected var dialogFragment: SimpleSamaDialogFragment? =
        SimpleSamaDialogFragment.new(getDialogLayout(), fullWidth, fullHeight)
            .with(getDialogDataBindingId(), bindingData ?: this)

    /**
     * Flag to know whether to automatically close the dialog when the activity is destroyed and to reopen it
     *  when the activity restarts (if instantiated through [SamaActivity.manageDialog]).
     */
     ...
     * If [enableAutoManagement] is set, or if the dialog was created through [SamaActivity.manageDialog],

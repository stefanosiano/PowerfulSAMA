    ...
    fun dismiss() {
        ...
        launch (Dispatchers.Main) {
            when (messageImpl) {
                ...
                MessageImpl.Snackbar -> { onDismiss?.invoke(this@Msg); (implementation?.get() as? Snackbar?)?.let { if(lockOrientation) unlockOrientation(it.context); if (it.isShown) it.dismiss(); waitForDismiss = false } }

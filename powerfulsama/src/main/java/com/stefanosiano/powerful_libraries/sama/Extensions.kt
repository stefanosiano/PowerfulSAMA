    ...
    distinctLiveData.addSource(this, object : Observer<List<T>> {
        ...
        private inline fun compareListsContent(list1: List<T>, list2: List<T>, compare: (T) -> Any): Boolean {
            for(i in list1.indices) {
                tryOrNull {
                    if(compare( list1[i] ) != compare( list2[i] )) return false
                } ?: return false
            }
            return true
        }
...
inline fun Observable.onChangeAndNow(c: CoroutineScope? = null, crossinline f: suspend () -> Unit) =
    onChange(c, f).also { launchOrNow(c) { f() } }
...
inline fun <T> tryOrPrint(default: T, toTry: () -> T): T {
    return try { toTry() } catch (e: Exception) { e.printStackTrace(); default }
}
...
inline fun <reified T : Enum<T>> String.toEnum(default: T) : T =
    try{ enumValueOf(this) } catch (e: Exception) { default }

...
class SamaObserverImpl: SamaObserver {
    ...
    /** Observes [o] until this object is destroyed and calls [obFun] in the background, now and whenever [o] or any of [obs] change, with the current value of [o]. Does nothing if [o] is null or already changed. Returns an [ObservableField] with initial value of null */
    override fun <R> observe(o: ObservableShort, vararg obs: Observable, obFun: (data: Short) -> R): ObservableField<R> = ObservableField<R>().also { toRet -> observePrivate<Short>(o, { toRet.set(obFun(it)) }, *obs) }
    ...
    override fun <R> observe(o: Flow<R>, vararg obs: Observable, obFun: (data: R) -> R): ObservableField<R> {
        ...
        synchronized(flows) {
            flows.addAll( obs.map { SamaInnerFlowObservable(it, it.onPropertyChanged { helper.onStart = f as (Any) -> Unit; helper.lastValue?.let { f(it) } }) } )
        }
    ...
    override fun startObserver() {
        ...
        synchronized(flowMap) { flowMap.values.forEach { coroutineScope?.launch { it.lastValue?.let { v -> it.onStart?.invoke(v) } } } }
    }
    ...
    private inner class SamaFlowHelper (val id: Int, var onStart: ((t: Any) -> Unit)?, var job: Job?, var lastValue: Any?)

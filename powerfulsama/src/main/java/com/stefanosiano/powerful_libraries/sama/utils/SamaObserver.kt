...
interface SamaObserver {
     ...
     *  multi-variables observe and to observe liveData on UI.
     */
     ...
     */
    fun <T> observeAndReloadLiveData(
        o: ObservableField<*>,
        vararg obs: Observable,
        f: () -> LiveData<T>?
    ): LiveData<T>
...
class SamaObserverImpl : SamaObserver {
    ...
    /** List of observableList callbacks that will be observed until the viewModel is destroyed. */
    ...
    override fun <R> observe(
        ...
        vararg obs: Observable,
        obFun: (data: Int) -> R
    ): ObservableField<R> = ObservableField<R>().also { toRet ->
        observePrivate<Int>(o, { toRet.set(obFun(it)) }, *obs)
    }
    ...
    override fun <R> observe(
        ...
        observePrivate<Short>(o, { toRet.set(obFun(it)) }, *obs)
    }
    ...
    override fun <R> observe(
        ...
        observePrivate<Long>(o, { toRet.set(obFun(it)) }, *obs)
    }
    ...
    override fun <R> observe(
        ...
        observePrivate<Byte>(o, { toRet.set(obFun(it)) }, *obs)
    }
    ...
    override fun <R> observe(
        ...
        observePrivate<Char>(o, { toRet.set(obFun(it)) }, *obs)
    }
    ...
    override fun <R> observe(
        ...
        observePrivate<Boolean>(o, { toRet.set(obFun(it)) }, *obs)
    }
    ...
    override fun <R> observe(
        ...
        observePrivate<Float>(o, { toRet.set(obFun(it)) }, *obs)
    }
    ...
    override fun <R> observe(
        ...
        observePrivate<Double>(o, { toRet.set(obFun(it)) }, *obs)
    }
    ...
    override fun <R, T> observe(
        ...
        observePrivate<T>(o, { toRet.set(obFun(it)) }, *obs)
    }

    /**
     * Observes [o] until the ViewModel is destroyed, using a custom observer, and calls [obFun].
     ...
     * Does not update the observable if the value of the liveData is null.
     */
    override fun <T> observeAsOf(liveData: LiveData<T>): ObservableField<T> =
        ObservableField<T>().also { ob ->
            ...
            o,
            {
                coroutineScope?.launch {
                    withContext(Dispatchers.Main) {
                        lastLiveData?.let { mediatorLiveData.removeSource(it) }
                    }
                    lastLiveData = f()
                    withContext(Dispatchers.Main) {
    ...
    override fun startObserver() {
        ...
        synchronized(observableMap) {
            observableMap.values.forEach {
                coroutineScope?.launch { it.onStart?.invoke() }
            }
        }
        synchronized(flowMap) {
    ...
    override fun destroyObserver() {
        synchronized(observables) {
            observables.forEach {
                it.ob.removeOnPropertyChangedCallback(it.callback)
            }
            observables.clear()
        }
        synchronized(flows) {
        ...
        synchronized(listObservables) {
            listObservables.forEach {
                it.ob.removeOnListChangedCallback(it.callback)
            }
            listObservables.clear()
        }
        (coroutineScope ?: GlobalScope).launch(Dispatchers.Main) {
            synchronized(customObservedLiveData) {
                customObservedLiveData.forEach {
    ...
    private inner class SamaInnerObservable(
        val ob: Observable,
        val callback: Observable.OnPropertyChangedCallback
    )

...
/** Obesrves a live data using a lambda function instead of an Observer (use this only if you don't need a reference to the observer */
fun <T> LiveData<T>.observe(lifecycleOwner: LifecycleOwner, observerFunction: (data: T?) -> Unit) = this.observe(lifecycleOwner, Observer { observerFunction.invoke(it) })
...
fun <T> LiveData<T>.getDistinct(): LiveData<T> {
    ...
    distinctLiveData.addSource(this, object : Observer<T> {
        ...
        override fun onChanged(obj: T?) {
            if(lastObj != null && obj?.equals(lastObj) == true)
                return
...
fun BaseObservable.addOnPropertyChangedCallback(onChanged: () -> Unit ){
...
fun <T> ObservableField<T>.addOnChangedAndNow(onChanged: (T?) -> Unit) { this.onChange{ onChanged.invoke(get()) }; onChanged.invoke(get()) }

/** Called by an Observable whenever an observable property changes. It also runs the same function now */
fun ObservableBoolean.addOnChangedAndNow(onChanged: (Boolean) -> Unit ) { this.onChange{ onChanged.invoke(get()) }; onChanged.invoke(get()) }

/** Called by an Observable whenever an observable property changes. It also runs the same function now */
fun ObservableInt.addOnChangedAndNow(onChanged: (Int) -> Unit ) { this.onChange{ onChanged.invoke(get()) }; onChanged.invoke(get()) }

/** Called by an Observable whenever an observable property changes. It also runs the same function now */
fun ObservableShort.addOnChangedAndNow(onChanged: (Short) -> Unit ) { this.onChange{ onChanged.invoke(get()) }; onChanged.invoke(get()) }

/** Called by an Observable whenever an observable property changes. It also runs the same function now */
fun ObservableLong.addOnChangedAndNow(onChanged: (Long) -> Unit ) { this.onChange{ onChanged.invoke(get()) }; onChanged.invoke(get()) }

/** Called by an Observable whenever an observable property changes. It also runs the same function now */
fun ObservableFloat.addOnChangedAndNow(onChanged: (Float) -> Unit ) { this.onChange{ onChanged.invoke(get()) }; onChanged.invoke(get()) }

/** Called by an Observable whenever an observable property changes. It also runs the same function now */
fun ObservableDouble.addOnChangedAndNow(onChanged: (Double) -> Unit ) { this.onChange{ onChanged.invoke(get()) }; onChanged.invoke(get()) }

fun BaseObservable.onChange(f:() -> Unit) { addOnPropertyChangedCallback(object : Observable.OnPropertyChangedCallback() {  override fun onPropertyChanged(sender: Observable?, propertyId: Int) { f.invoke() } })}
...
/**
 * Run a function for each element and wait for the completion of all of them.
 * Uses coroutines
 */
fun <T> Iterable<T>.runAndWait(run: (x: T) -> Unit) {
    runBlocking {
        map { async {run.invoke(it)} }.map { it.await() }
    }
}
...
/** Wraps the lambda function [toTry] in a try catch block, return null if an exception is raised */
inline fun <T> tryOrNull(toTry: () -> T): T? = tryOr(null, toTry)

/** Wraps the lambda function [toTry] in a try catch block, return [default] if an exception is raised */
